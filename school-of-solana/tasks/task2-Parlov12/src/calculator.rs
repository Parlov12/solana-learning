...
impl OperationType {
    ...
    pub fn perform(&self, x: i64, y: i64) -> Option<i64> {
        match self {
            OperationType::Addition => x.checked_add(y), // checks for overflow - return Some(result) or None
...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let op_type = OperationType::Addition; // create correct operation
        let result: Option<i64> = op_type.perform(x, y); // use method implemented in operation

        if let Some(value) = result {
            let operation = Operation::new(x, y, op_type);
            self.history.push(operation);
            Some(value) // returning result
        } else {
            None // returnin None
        }
    }
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        let result: Option<i64> = op_type.perform(x, y);
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        if let Some(value) = result {
            let operation: Operation = Operation::new(x, y, op_type);
            self.history.push(operation);
            return Some(value);
        } else {
            None
    ...
    pub fn show_history(&self) -> String {
        ...
        for operation in &self.history {
            if let Some(result) = operation.operation_type.perform(operation.first_num, operation.second_num) {
                let line : String = format!( // format! - used to form a string
                    "{}: {} {} {} = {}\n",
                    i,
                    operation.first_num, // first number
                    operation.operation_type.get_sign(), // get_sign() - method implemented for OperationType
                    operation.second_num, // secound number
                    result // result we assigned in for loop
                );
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        if let Some(operation) = self.history.get(operation_index) { // if using &self.history[operation_index] - program "panics" if index is invalid
        let result = operation.operation_type.perform(operation.first_num, operation.second_num)?; // "?" returns None early if result is None
            
            let repeated_operation = operation.clone(); // it must be clone because of ownership
